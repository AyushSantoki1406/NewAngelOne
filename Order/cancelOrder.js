    ...
    const results = await Promise.all(
      credentials.map(async (cred) => {
        try {
          ...
          // Step 1: Filter the orders based on provided fields
          ...
          console.log("filteredOrders ", orderData);
          // Step 2: Cancel the filtered orders using orderid
          const cancelPromises = filteredOrders.map((order) =>
            header(
              "post",
